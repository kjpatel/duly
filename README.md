...
- [ ] **Contract closeout** — all three questions are decided in [spec/compatibility.md](spec/compatibility.md); what remains is the one that needs code. Quantified bindings are **deferred past v1.0** (C5): no committed pack needs them, re-establishing `prove`'s decidable fragment is not freeze-sized work, and a new semantics version is what makes deferral safe rather than debt. The run envelope **reserves no signature affordance** (C7), because a signature inside a hashed body is circular and a sidecar is the same idiom the receipt already uses. A review resolution **must supersede the fact it rules on** for `low_confidence` items (C6) — decided, with `duly_review` still to enforce it.
...
- [x] **Compatibility policy** ([spec/compatibility.md](spec/compatibility.md)) — what v1.0 promises per contract, what it does not cover, and the two claims that needed code: replay refuses semantics it does not implement, and `decision_digest()` defines when two receipts record the same adjudication.
