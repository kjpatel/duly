...
Usage (from the repo root): uv run python examples/starters/tools/check_facts.py
"""
...
# Relative to this script's own tree, not to a counted-out repo root: the
# starters moved once already (under examples/), and a parent-count breaks
# silently on the next move while `tools/..` cannot.
...
def main() -> int:
    ...
    for manifest_path in sorted(STARTERS.glob("*/scenario.json")):
        ...
        print(f"scenario {manifest['id']} ({manifest_path.relative_to(STARTERS.parent)})")
