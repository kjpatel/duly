...
MORTGAGE = REPO / "ontologies" / "duly-mortgage-closing" / "0.1.0.yaml"
NOTICE = REPO / "ontologies" / "duly-starter-notice" / "0.1.0.yaml"
...
def _committed_fee_type_fact() -> dict:
    return json.loads(
        (REPO / "starters/trid/facts/fact-cd-fee-type.json").read_text(encoding="utf-8")
    )
...
def test_shacl_coexists_with_the_provo_export(mortgage_shapes):
    ...
    local = {
        url: REPO / "spec" / "contexts" / url.rsplit("/", 1)[-1]
