...
- [ ] Z3/OR-Tools for arithmetic and timing constraints. The consumer exists: [tila-rescission](rulepacks/tila-rescission-us-federal/pack.yaml) carries a `MODELING BOUNDARY` comment because the IR cannot walk a calendar, so a rescission deadline enters as an extracted fact certified against a necessary condition instead of being computed. Rewriting that pack to compute what it currently documents is the acceptance test. Constraint: solver output must be deterministic across versions and equisatisfiable models, or replay breaks — the reason this is a milestone item and not a patch
