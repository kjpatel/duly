...
- [ ] PROV-O JSON-LD context for facts and receipts. *What it buys:* duly's provenance chain — fact ← rendition ← document, decision ← rule-pack version — becomes readable by any RDF/SPARQL tool via one small mapping file, with stored documents unchanged (they are content-addressed; the context is applied externally). For an enterprise whose data-governance stack already speaks W3C PROV, "apply this context" replaces "write an importer", and an auditor can ask *which decisions ever used a fact from this document?* with off-the-shelf tooling. It also converts this README's standards-alignment claim from asserted to demonstrated — and the mapping's "deliberately not mapped" list (bitemporal effective time and calibrated confidence have no PROV equivalent) becomes a precise statement of where duly is novel versus standard
