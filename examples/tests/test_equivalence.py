...
`uv run pytest examples/tests -q`. Both packs compared below are teaching
content, so the comparison goes with them; the compiler's own contract lives
in `dmn/tests/` on `fixtures/dmn`.

`examples/dmn/trid-fee-tolerance.dmn` is the three rules of
`examples/rulepacks/trid-fee-tolerance-us-federal/pack.yaml` re-authored as two
DMN decision tables. These tests adjudicate the committed starter facts with both
...
def facts():
    ...
    assert facts, f"no committed facts in {TRID_FACTS}"
    return facts
...
def test_the_packs_expected_yaml_cases_hold_for_the_compiled_pack(facts, dmn_pack):
    ...
    hand-written pack in `examples/tests/test_rulepacks.py` — run here against
    the compiled one."""
...
def test_the_committed_compilation_is_what_the_dmn_compiles_to(dmn_pack):
    """The .pack.yaml in examples/dmn/ is a build artifact under version
...
def test_the_spec_demo_runs_and_reports_equivalence():
    ...
    proc = subprocess.run(
        [sys.executable, str(REPO_ROOT / "spec" / "dmn_demo.py")],
