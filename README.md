...
| [Extraction adapters](extraction/) | Document AI providers emitting contract-conformant facts with verified run envelopes ([the protocol, the acceptance bar, and what is not provided](extraction/README.md)) | Docling + scripted stub; commercial adapters deferred to M6 |
...
- [x] **Adopter's guide** — [docs/adopters-guide.md](docs/adopters-guide.md): documents, extraction adapter, grounded facts, ontology conformance, rule packs, golden corpus, review queue, and calibration labels — written by executing every command against the installed wheel, then **executed cold by an agent with no repository context and an invented domain**, which reached a verified, replaying corpus in one sitting and whose findings drove three shipped fixes.
- [x] **Contribution paths, both edges** — the pack edge in [examples/rulepacks/README.md](examples/rulepacks/README.md)'s "Contributing it back"; the adapter edge in [extraction/README.md](extraction/README.md), whose central observation replaces a recording harness: conversion and proposal are separable stages with a content-addressed artifact between them, so the rendition *is* the recording.
...
- **Extraction adapters** — vendors and users of a given document AI service can maintain its adapter independently, the way observability vendors maintain OpenTelemetry exporters. The path: [extraction/README.md](extraction/README.md) for the protocol, the five-property acceptance bar and the boundaries, [CONTRIBUTING.md](CONTRIBUTING.md#contributing-an-extraction-adapter) for the checklist.
...
**M0 through M5 are complete** ([releases](https://github.com/kjpatel/duly/releases), current v1.1.1). The toolkit is installable (a plain install is seven packages, asserted at build time), deletable (`git rm -r examples/` leaves 791 toolkit tests passing — enforced by CI on every push), documented for adoption (a bring-your-own guide executed cold by an agent with no repository context), measured ([the capacity envelope](docs/capacity-envelope.md)), and accountable for its contracts ([the stability policy](spec/compatibility.md)). 351 golden decisions replay byte-for-byte on every push, unregenerated through the whole milestone.

**Next ([M6](#m6--durable-deployment-and-extraction-evaluation)): a first adoption, reported** — the evidence the guides worked that no amount of self-testing can manufacture — then durability and extraction evaluation, each gated on a deployment that needs it.
