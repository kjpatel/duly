...
- [x] **Minimal integration example** ([examples/minimal-integration](examples/minimal-integration/)) — three facts, three rules, one adjudication, one verified receipt in author-owned code, run from an installed wheel with duly's source tree absent. Before the guide, not after: it is the cheapest test that the published seam is importable at all, and writing it found what the walkthrough will have to route around — a first-contact API reachable only by a private-looking path, a CLI defaulting to a repo-relative directory, and a kernel-only install that pulls in a web framework.
