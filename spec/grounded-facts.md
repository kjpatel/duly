...
Schemas: [`grounded-fact.schema.json`](../core/duly_core/schemas/grounded-fact.schema.json), [`decision-receipt.schema.json`](../core/duly_core/schemas/decision-receipt.schema.json), [`extraction-run.schema.json`](../core/duly_core/schemas/extraction-run.schema.json) — they live inside [`duly_core`](../core/duly_core/) rather than under `spec/` because they **ship in the wheel**: two packages read them at runtime, and a path relative to a package resolves the same in a checkout and in site-packages, where a path relative to the repository does not. `duly_core.load_schema("grounded-fact")` is the accessor.
...
- **`abstentions`** — attributes the case declined to use as asserted (missing, below confidence policy, or conflicting), with routing. The exclusion is case-wide, not per-question: the kernel filters the case's live facts before any rule runs, so the same entries appear on every receipt for the case — including decisions whose rules never consult the attribute. Abstention appears on the receipt, not on the fact (D5).
...
3. **PII sensitivity** — facts carry an optional `sensitivity` field (`public` | `internal` | `pii`). Renderers must redact the `quote` of a `pii` fact in human-facing output while preserving the document reference, span, and content hash — the evidence chain survives redaction. The default (absent field) is `internal`. The field is *declared*, never inferred: an extraction target names the handling class and the adapter carries it through verbatim — an adapter guessing at PII would be asserting a handling class it cannot ground.

4. **Batch envelope** (deferred from M0, resolved in M3) — an extraction run emits one **envelope** per rendition: a manifest `{runId, adapter{name, version, modelId?}, documentId, documentSha256, rendition{id, sha256}, createdAt, factIds[]}` where `factIds` is the ordered, complete list of facts the run proposed and `rendition.sha256` is the SHA-256 of the rendition text's UTF-8 bytes — the content address D4 promised the rendition would have. The envelope is content-addressed exactly like facts (D8: SHA-256 over the JCS canonical form excluding `id` and `contentHash`; id = `urn:duly:run:sha256:<hash>`), so consumers can verify a whole run before ingesting — manifest hash, every fact's hash, every span against the rendition — and revoke a whole run at once by retracting every live fact carrying its `runId` (retraction is an event, never a deletion, per D7). "Signed" at this stage means that integrity hash: tampering with the manifest, any fact, or the rendition is detectable, but there is no authenticity claim about *who* produced the run — asymmetric signatures are the open question below. **Why one envelope per rendition rather than per case or per batch:** the rendition is the unit of span validity (D4) and the document hash is the unit of source integrity (D3); a run over several documents emits several envelopes. **Rejected:** per-fact signatures (N artifacts per document and no way to assert "this run is complete — nothing was dropped"), and embedding run membership in the fact beyond `runId` (facts are already content-addressed; run membership is the envelope's job, and moving it into the fact would change every fact hash whenever the run's membership was corrected). Schema: [`extraction-run.schema.json`](../core/duly_core/schemas/extraction-run.schema.json); example: [`examples/envelopes/envelope-decpage-run.json`](examples/envelopes/envelope-decpage-run.json) (kept in a subdirectory so fact-fixture tooling globbing `examples/*.json` is unaffected); producer and consumer: [`extraction/duly_extraction/`](../extraction/duly_extraction/).

## Open questions

1. **Envelope signatures** — asymmetric signatures over the envelope's canonical bytes would add authenticity — *who* ran the extraction — on top of the integrity hash resolved question 4 provides. The scheme is still deferred until a deployment needs cross-organization trust: key distribution and rotation are a heavier commitment than any current consumer justifies. What v1.0 settled is the **shape** ([compatibility.md](compatibility.md) C7): signatures never go inside the envelope. An optional `signatures` array in the hashed body is circular — `contentHash` covers every field, so storing the signature changes the bytes that were signed — and excluding it from the hash would make the hash cover less than the document. So a signature travels in a separately-hashed sidecar keyed by the envelope's `contentHash`, the envelope reserves no affordance for one, and "adding signatures later breaks nothing" is now a mechanism rather than a hope.

2. ~~**Must a review-queue correction supersede the below-floor fact it rules on?**~~ **Decided: yes, for `low_confidence` items** ([compatibility.md](compatibility.md) C6). Resolving one is, by construction, a ruling on that specific fact, and `supersedes` is the truthful record of that act; it also makes the state that bends this spec's own definition of `abstentions` ("attributes the case declined to use as asserted" — under the coexisting form the attribute *was* used) unrepresentable rather than worked around, and ties calibration labels to the machine fact structurally instead of by store-history lookup at resolve time. Nothing is lost: the superseded fact, its score and the chain survive in history and in knowledge-time replay.

   The evidence, more than the argument: `review-0001` — the preserved-forever golden case — already uses the superseding form, and its receipt's `abstentions` array is empty. The coexisting branch was exercised by no committed artifact.

   Scope, deliberately narrow. Enforcement sits at the **queue** boundary, which refuses a `low_confidence` resolution whose correction does not supersede the item's abstained fact and names the fact id it must carry — it cannot *stamp* the field, as this question's earlier sketch proposed, because a correction arrives content-addressed and writing into it would change its identity; the **store keeps accepting independent human facts**, because a value known from a phone call is not a ruling on an extraction; and **`conflict` items are not covered** — their entries carry several facts and the analogous rule is not obvious, so the current behaviour stands there until someone has a reason. It is a hard invariant of what "resolve" means, not a pack-versioned knob: a knob would reintroduce the contradictory state under configuration. One real cost: supersession requires the store to have held the fact, which the envelope-ingest pipeline satisfies and the demo's from-disk facts do not — a boundary of where the queue applies, not a defect in the rule.
