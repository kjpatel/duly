...
- [x] DMN decision-table authoring that compiles to the existing IR ([spec/dmn.md](spec/dmn.md), [demo](spec/dmn_demo.py), `python -m duly_dmn`). A DMN-authored pack produces the same decision, the same rules fired in the same order with the same defeat chains, and the same input facts as its hand-written equivalent — proven against the TRID pack in `dmn/tests/test_equivalence.py`. **The receipt hash necessarily differs**, and the original acceptance criterion here ("the same decision *and receipt*") was unachievable: a receipt pins its rule pack's name and version, so two packs are two identities. The test asserts that difference as precisely as it asserts the agreement. The compiler refuses rather than approximates — cells outside S-FEEL, hit policies the IR cannot express, and uncited or undated rows are compile errors naming the row and the cell.
- [x] Z3 static pack verifier ([spec/pack-verification.md](spec/pack-verification.md), [demo](spec/prove_demo.py), `python -m duly_assurance prove`). For every pair of same-priority rules concluding one attribute it returns PROVED-DISJOINT, NOT-PROVED with a concrete witness, or OUT-OF-FRAGMENT naming the construct it could not encode; it also reports uncovered input regions per decision attribute, and `prove-equivalent` compares two packs over the input space rather than a fixture list. Across the six committed packs: 25 pairs proved disjoint, one genuine overlap (correctly — a registered eNote *is* a promissory note, which is why `PKG-NOTE-31 overrides PKG-NOTE-30`), nine uncovered regions, all intended and now witnessed. It proves what the syntactic validator structurally cannot — boolean splits, numeric ranges, and guards on *derived* values — including the TILA pack's two disjointness claims that previously lived only in code comments, one of them over a deadline the pack computes with its own business-day calendar. **`validate_pack` is deliberately not relaxed**: the kernel's proof set stays small enough to audit by reading. Validation-time only; `z3-solver` is an optional extra and no solver output participates in a receipt.
- [x] Pack-owned decision phrasing, so non-boolean decisions do not require a core or demo code change ([spec/rule-ir.md](spec/rule-ir.md), "Decision phrasing"). Six packs, fourteen decisions: every determination the demo can render is byte-identical before and after, now from pack data alone — the wording moved without a word of it changing.
- [x] Rule-ID convention and contribution checks before further packs entrench incompatible conventions ([spec/rule-ir.md](spec/rule-ir.md), "Rule ids"). Forward-looking by construction: **nothing is renamed**, because `NY-NR-45` sits in 76 golden receipts and a receipt is not editable. A rule id is a handle, not a claim — everything an id is tempted to encode (statute, date, threshold, jurisdiction) already has a correctable home on the rule, and the id does not. The 46 existing ids are exempt by an explicit committed list; 17 of them would fail the convention today.
- [x] OR-Tools scheduling example ([examples/closing-scheduler](examples/closing-scheduler/), `uv run --with ortools python examples/closing-scheduler/schedule.py`). CP-SAT plans a mortgage closing to the earliest feasible sign → fund → record dates. Every hard constraint is a table of days — and of `(signing, funding)` pairs, since the rescission deadline is computed *from* the consummation date — that `adjudicate` actually permitted; the only constraints the scheduler writes itself are availability, labelled synthetic. **Each chosen date cites the receipt ids that constrained it**, and funding carries its boundary: the receipt permitting 2026-02-21 and the receipt refusing 2026-02-20. The plan funds Monday 2026-02-23 because compliance opens *Saturday* 2026-02-21 — Saturdays are business days under the precise § 1026.2(a)(6) calendar, the rule schedulers get wrong — and the wire desk is closed Saturdays; the output says which of the two bound the date. **The separation is checked by mutation:** moving the TILA pack from three business days to five moves the plan with `schedule.py` untouched, and opening the wire desk on Saturdays snaps the plan onto the untouched compliance floor.
...
- [x] Z3 what-if queries ([spec/whatif.md](spec/whatif.md), [demo](spec/whatif_demo.py), `python -m duly_whatif`). Free one input of a decided case and solve the pack backwards: *the notice had to be mailed by 2026-04-24*; *the fee could reach 2984.02 before a cure is owed*; *funds may not move until 2026-02-18* — that last one over a rescission deadline the TILA pack computes itself, on a calendar where Saturday counts as a business day and Washington's Birthday does not. **Every answer is re-adjudicated through the kernel before you see it**, and extremals are boundary-verified: the value passes and one step beyond it fails. A solver/kernel disagreement raises with both artifacts rather than returning an unverified answer — and a deliberately broken encoding, committed as a test, proves that guard actually trips. It reuses the verifier's SMT encoding unmodified; a test asserts class identity so divergence must be deliberate..
