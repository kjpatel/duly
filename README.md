...
- [ ] OR-Tools scheduling example — reference wiring for duly inside a larger system: adjudications produce the permissibility windows (clear-to-fund, RON-permitted dates, recordable), an optimizer chooses among permissible options, and **no compliance rule is ever re-encoded in the scheduler**. The separation is the point: duly decides what is *allowed*, the solver decides what is *best*, and the chosen plan cites the receipts that constrained each date.
