...
1. **Should a proved-disjoint pair be recordable in the pack?** P1 sketches what that would take. The blocking question — what a replaying kernel does with a declared proof it cannot re-verify — is the same shape as [spec/dmn.md](dmn.md) open question 1 (`rulePack.compiledFrom`) and the M5 backend-identity question, and should be decided with them.
2. **Should `prove` run in CI over `rulepacks/`?** It is fast enough (all six packs in well under a second) and it exits zero today, so adding it costs nothing and would catch a same-priority overlap the moment `_equality_guards` stopped being sound. The argument against is that it makes an optional dependency a required CI install for every PR touching a pack. Not decided.
