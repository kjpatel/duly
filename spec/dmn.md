...
1. **Should a compiled pack record its provenance on the receipt?** Today the only trace is a comment in the emitted YAML, which is outside `rulePack.version` and therefore outside the receipt. A `rulePack.compiledFrom` field (source path + source hash) would make "this decision came from a DMN table, and here is the table's hash" replayable. It is additive but it changes receipt bytes, so it wants deciding alongside the M5 backend-identity question, not before it.
