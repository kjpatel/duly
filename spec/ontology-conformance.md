...
[Grounded-facts D10](grounded-facts.md#d10-ontology-by-reference) made a promise with two halves: facts reference the user's ontology by CURIE + version, and duly *validates conformance* while defining no domain terms. This document is the second half made concrete: what an ontology artifact IS in duly, the two committed samples ([`examples/ontologies/`](../examples/ontologies/)), and the gate ([`conformance/duly_conformance`](../conformance/duly_conformance/)) that rejects a nonconforming fact loudly at the contract line.
...
An ontology in duly is one LinkML schema file at `<registry>/<name>/<version>.yaml` (this repo's samples: `examples/ontologies/`), immutable once committed — exactly the discipline rule packs follow. A fact pinning `{ontology: duly-mortgage-closing, version: "0.1.0"}` validates against that file's committed bytes forever; changes go in a new version file beside it.
...
**Why the insurance name did not consolidate too:** `examples/golden/cases/review-0001` is the repo's only human-provenance golden case — born from a real review-arc correction, preserved forever because no seed can regenerate it ([examples/golden/README.md](../examples/golden/README.md)). Its facts pin `duly-starter-notice@0.1.0` inside their content hashes; renaming the ontology would strand them. The name is imperfect and kept deliberately — an honest cost of the immutability rule, recorded here rather than papered over.
...
- **CLI** — `uv run python -m duly_conformance --ontologies DIR check <paths...>` sweeps fact files or directories; `... list` prints the registry. The registry directory has no default (`DULY_ONTOLOGIES` also works): duly does not know where an adopter keeps their ontologies, and guessing its own layout is right exactly once (every class, attribute, kind, and code set). The repo-wide guarantee — every committed fact conforms — is a standing test ([`examples/tests/test_example_conformance.py`](../examples/tests/test_example_conformance.py)); it lives with the content it sweeps, because both the facts and the ontologies they cite are this repository's teaching examples. The gate's own contract — the subset parser, the check rules, the CLI's exit codes — is `conformance/tests/` and stands on its own.
