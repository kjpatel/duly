...
The derivation above it gains a matching **Input excluded** card, naming the rule that went unevaluated for want of that fact. It sits *after* the tree rather than inside it, because the tree is the receipt's own derivation and a synthetic node within it would be the page inventing a step the kernel never took. Ask the LA County scenario's SB 2 fee question to see the other half: the same case still carries the same exclusion, but no rule behind that answer reads it, so the section folds it away under one line instead of showing it as a peer of the evidence that decided.
